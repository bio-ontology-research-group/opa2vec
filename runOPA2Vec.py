...
cmdonto ="groovy ProcessOntology.groovy " + str(ontology_file) +" "+str(reasoner) + " "+ str(axiomsinf.name)+" "+str(axiomsorig.name)+" "+str(classesfile.name)
os.system(cmdonto)
cmdMerge ="cat " +str(axiomsorig.name)+" "+str(axiomsinf.name)+" >"+str(axioms.name)
os.system(cmdMerge)
cmdgetclas="perl getclasses.pl "+str(association_file)+" "+str(annot_classes.name)
...
cmdmeta ="groovy getMetadata.groovy "+ str(ontology_file)+" "+str(listofuri)+" " + str(metadatafile.name)
os.system(cmdmeta)
if (annot == 'yes' or annot == 'Yes' or annot == 'YES'):
	print ("\n 	2.1. Metadata annotation ...")	
	cmdannotat="perl AnnotateMetadata.pl" + str(metadatafile.name) + "> " +str(annotated_meta.name)
	os.system(cmdannotat)
print ("\n   ######################################################################\n")

#3. Process associations
print "\n		3.Propagate Associations through hierarchy ...\n"
cmdanc="perl AddAncestors.pl "+ str(association_file)+ " "+str(axioms.name)+" "+str(classesfile.name)+" "+ str(AssoAxiomInferred.name)
os.system(cmdanc)
...
tempoutfile ="tempout"
print "\n		5.Running Word2Vec ... \n"
cmdwv="python2.7 runWord2Vec.py "+str(finalclasses.name)+" "+str(window)+" "+str(embedding)+" "+str(mincoun)+" "+str(model)+" "+str(pretrained)+" "+ str(ontology_corpus.name)+" "+ str(tempvecs.name)
os.system(cmdwv)
